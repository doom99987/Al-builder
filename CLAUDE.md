...
**Messages are disabled** (2026-09-24, owner request). The floating Messages button and the DM panel are commented out in `index.html` ("Messages are disabled"), and `DM_ENABLED = false` in `js/trades.js` stops every DM path (open, pop-out, realtime subscription, unread badge, the "open chat" link on old trade notifications); the overlay's Party Chat tab is commented out in `js/overlay.js`. The notification bell and the site consent prompt still run from `trades.js`. The `direct_messages` table is untouched. To restore: uncomment the block, set `DM_ENABLED = true`, put the overlay tab back, bump the stamps and `SITE_VERSION`.
...
SQL lives in `supabase/*.sql`, but **only some tables are checked in** — `banks.sql`, `builds.sql`, `matchmaking.sql`, `reports.sql`, `qte-scores.sql` (the leaderboard RPCs, added after they silently ate scores for months) `lockdown.sql` (the score and ban tables made read-only through the API, and every admin action as an admin-checked RPC) and `lockdown2.sql` (own-row policies for profiles, notifications, direct messages, trade listings, the four party tables, shared builds and the avatars bucket — every other table the client writes; column-level update grants; a `stamp_profile_name` trigger that overwrites every denormalised display name from `profiles`, so a client-sent `username`/`sender_name`/`host_name` is never trusted). The tables themselves still exist only in the dashboard. If a change needs schema you cannot see, ask rather than guess.

**Who is an admin is the server's answer, and `isAdmin()` in `sb.js` only decides which buttons to draw.** The admin list is the table `public.site_admins` (`supabase/admin-server.sql`) - no API role can read it, `site_admin_ids()` is not callable from the API, and no file in the repo names an admin (a test enforces it; add one in the SQL editor: `insert into public.site_admins (user_id) values ('<uuid>')`). `sb.js` asks `is_site_admin()` at sign-in and again before opening the panel; new reports ring the admins from a database trigger. A player who flips the flag in dev tools gets a panel whose buttons all fail and whose admin-only tables answer nothing. Any table the admin panel could write directly, every signed-in user could write from a console — that is how scores were injected and accounts banned from the outside. So the client never writes `leaderboard`, `leaderboard_records`, `personal_bests`, `banned_usernames`, `perma_banned_usernames` or `qte_sessions` (a test enforces it); it calls `admin_*` RPCs, each of which checks `is_site_admin()` on the server. A new admin action is a new SECURITY DEFINER function in `lockdown.sql` with that check, revoked from `public` and granted to `authenticated` — never a direct table write. A ban also sets `auth.users.banned_until`, so it holds without the login form's own check.
...
| `leaderboard`, `leaderboard_records`, `personal_bests`, `score_reviews` (held scores awaiting an admin) | `sb.js` |
